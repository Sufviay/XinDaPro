...
class ReviewStarTFView: UIView, UITextViewDelegate {

    var clickStarBlock: VoidBlock?
    
    var inputBlock: VoidBlock?
    
    var type: String = "" {
        didSet {
            if type == "1" {
                //外卖
                self.tLab.text = "Delivery time"
            }
            if type == "2" {
                self.tLab.text = "Collection time"
            }
        }
    }
    ...
    private let titLab: UILabel = {
        ...
        lab.text = "Are you satisfied with our delivery/pickup?"
        return lab
    }()
    
    private let sImg: UIImageView = {
        let img = UIImageView()
        img.image = LOIMG("review_ps")
        return img
    }()
    ...
    private let hLab: UILabel = {
        ...
        lab.text = "Praise it if you like"
        return lab
    }()
...
//        }
        ...
        let backView = UIView()
        backView.backgroundColor = HCOLOR("#F7F7F7")
        ...
        backView.snp.makeConstraints {
            ...
            $0.top.equalTo(sImg.snp.bottom).offset(20)
