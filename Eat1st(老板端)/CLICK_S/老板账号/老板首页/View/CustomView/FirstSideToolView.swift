...
class FirstSideToolView: UIView, UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource, SystemAlertProtocol {
    ...
    private let imgStrArr: [String] = ["side_home", "side_set", "side_review", "side_tongzhi", "side_order", "side_customer", "side_coupon", "side_tousu", "side_setting"]

    private let titStrArr: [String] = ["Home".local, "Restaurant settings".local, "Reviews".local, "Operation Record".local,  "Orders".local, "Customer Management".local, "Promotion Management".local, "Complaints".local, "System settings".local]
    ...
    @objc func longpressAction() {
        ...
        let date = UserDefaults.standard.loginDate ?? ""
...
extension FirstSideToolView {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 3 {
            ...
            PJCUtil.currentVC()?.navigationController?.pushViewController(msgVC, animated: true)
