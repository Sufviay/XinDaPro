...
class BusyTimSelectAlert: UIView, UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    private let backView: UIView = {
        ...
        view.cornerWithRect(rect: CGRect(x: 0, y: 0, width: S_W, height: bottomBarH + 550), byRoundingCorners: [.topLeft, .topRight], radii: 10)
        return view
    }()
    ...
    private let titlab: UILabel = {
        ...
        lab.text = "Business Status Settings".local
        return lab
    }()
    
    private let line: UIImageView = {
        let img = UIImageView()
        img.image = GRADIENTCOLOR(HCOLOR("#2B8AFF"), HCOLOR("#28B1FF"), CGSize(width: 70, height: 3), 0)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.setCellData(titStr: dataArr[indexPath.row].busyName, isSelect: isSel)
        return cell
    }
    ...
    func setAlertData(data: [BusyTimeModel], idx_s: Int) {
        dataArr = data
        selectIdx = idx_s
        table.reloadData()
    }
    ...
    private func setBusyTime_Net() {
        HUD_MB.loading("", onView: PJCUtil.getWindowView())
        HTTPTOOl.setBusyTime(busyID: dataArr[selectIdx].busyId).subscribe(onNext: { [unowned self] (json) in
