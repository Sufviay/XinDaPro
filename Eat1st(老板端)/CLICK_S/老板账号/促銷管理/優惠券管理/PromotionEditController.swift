...
class PromotionEditController: LBBaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 6 {
            let h = "When the minimum eligible order amount is greater than 0, the discount amount must be less than this value.".local.getTextHeigh(TXT_10, S_W - 40) + 15
            return h
        }
        ...
        if indexPath.section == 13 {
            let h = "The number of valid days is calculated by adding a specified number of days to the issue date.".local.getTextHeigh(TXT_10, S_W - 40) + 15
            return h
        }
