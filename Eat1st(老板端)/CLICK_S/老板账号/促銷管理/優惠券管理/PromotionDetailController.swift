...
class PromotionDetailController: LBBaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            let h = dataModel.couponName.getTextHeigh(TXT_14, S_W - 80)
            return h + 50
        }
        
        if indexPath.section == 6 {
            let h = dataModel.dishesArr[indexPath.row].getTextHeigh(TXT_14, S_W - 125) + 20
            return h
        }
        
        if indexPath.section == 11 {
            let h = dataModel.tagStr.getTextHeigh(TXT_14, S_W - 80)
            return h + 50
        }
