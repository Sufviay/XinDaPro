...
class OnlineStatusCell: BaseTableViewCell {
    ...
    private let statusImg: UIImageView = {
        let img = UIImageView()
        img.image = GRADIENTCOLOR(HCOLOR("#FFC65E"), HCOLOR("#FF8E12"), CGSize(width: 50, height: 20), 0)
    ...
    func setCellData(isOnLine: Bool) {
        if isOnLine {
            self.statusImg.image = GRADIENTCOLOR(HCOLOR("#FFC65E"), HCOLOR("#FF8E12"), CGSize(width: 50, height: 20), 0)
            self.statusLab.text = "Open"
            self.statusLab.textColor = .white
        } else {
            self.statusImg.image = GRADIENTCOLOR(HCOLOR("#F8F9F9"), HCOLOR("#F8F9F9"), CGSize(width: 50, height: 20), 0)
...
class LiveRepotingCell: BaseTableViewCell {
    ...
    private let line: UIImageView = {
        ...
        img.image = GRADIENTCOLOR(HCOLOR("#FFC65E"), HCOLOR("#FF8E12"), CGSize(width: 70, height: 3), 0)
        return img
    }()
    ...
    func setCellData(titStr: String) {
        ...
        if titStr == "Live Reporting" || titStr == "Sales".local {
            line.image = GRADIENTCOLOR(HCOLOR("#FFC65E"), HCOLOR("#FF8E12"), CGSize(width: 70, height: 3), 0)
        }
        if titStr == "Real time order" || titStr == "Expenditure" {
            line.image = GRADIENTCOLOR(HCOLOR("#2C6BF8"), HCOLOR("#6B12CC"), CGSize(width: 70, height: 3), 0)
        }
