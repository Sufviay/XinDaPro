...
class ReviewsHeaderCell: BaseTableViewCell {
    ...
    private let line1: UIImageView = {
        let img = UIImageView()
        img.image = GRADIENTCOLOR(HCOLOR("#FF8E12"), HCOLOR("#FFC65E"), CGSize(width: 70, height: 3), 0)
    ...
    private let line2: UIImageView = {
        let img = UIImageView()
        img.image = GRADIENTCOLOR(HCOLOR("#28B1FF"), HCOLOR("#28B1FF"), CGSize(width: 70, height: 3), 0)
        img.clipsToBounds = true
        img.layer.cornerRadius = 1
        return img
    }()
