...
class PromotionInfoCell: BaseTableViewCell, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.section == 0 {
            let h = dataModel.couponName.getTextHeigh(TIT_18, S_W - 100) + 25
            return h
        }
        ...
        if indexPath.section == 2 {
            let h = dataModel.couponContentStr.getTextHeigh(TIT_14, S_W - 40) + 35
            return h
        }
        
        if indexPath.section == 3 {
            let h = dataModel.ruleStr.getTextHeigh(TIT_14, S_W - 40) + 35
            return h
        }
