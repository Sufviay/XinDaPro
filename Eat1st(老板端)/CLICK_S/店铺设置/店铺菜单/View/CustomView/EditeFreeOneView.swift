//
//  EditeFreeOneView.swift
//  CLICK_S
//
//  Created by 肖扬 on 2023/12/12.
//

import UIKit
import RxSwift

class EditeFreeOneView: UIView, UIGestureRecognizerDelegate {
    
    private var oldStatus: Bool = false

    private var isGiveOne: Bool = false {
        didSet {
            if isGiveOne {
   ...
   func disAppearAction() {
    ...
    func setAlertData(isGive: Bool, dishId: String) {
        oldStatus = isGive
        isGiveOne = isGive
        dishesID = dishId
    }
    ...
    private  func setGiveFreeOne_Net() {
        ...
        HTTPTOOl.doGiveOne(id: dishesID).subscribe(onNext: { [unowned self] (json) in
