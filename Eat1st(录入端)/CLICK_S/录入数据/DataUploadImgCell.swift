...
class DataUploadImgCell: BaseTableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, SystemAlertProtocol, CommonToolProtocol, UIImagePickerControllerDelegate, UINavigationControllerDelegate, PHPickerViewControllerDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.item == picImgArr.count {
            ...
            showSelectImgSheetAlert(titStr: "Choose picture", xcAction: { [unowned self] in
                print("album")
                self.showPhotoLibrary((9 - self.picImgArr.count), 0)
            }) { [unowned self] in
                print("camera")
                self.showCamera()
            }
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        for result in results {
            ...
            let pro = result.itemProvider
            pro.loadObject(ofClass: UIImage.self) { img, error in
