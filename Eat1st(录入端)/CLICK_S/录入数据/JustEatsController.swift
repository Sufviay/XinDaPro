...
class JustEatsController: HeadBaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private let name1Arr = ["Orders", "Card(£)", "Cash(£)", "Top up(£)", "Total(£)"]
    private let name2Arr = ["訂單數", "卡支付", "現金支付", "充值", "合計"]
...
//        if ptModel.platId == 6 {
//            
//            if ptModel.orders != "" && ptModel.card != "" && ptModel.cash != "" && ptModel.total != "" && ptModel.topUp != "" {
//                
...
//            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 3 {
            content = ptModel.topUp
        }
        if indexPath.section == 4 {
            content = ptModel.total
        }
        cell.setCellData(name1: name1Arr[indexPath.section], name2: name2Arr[indexPath.section], content: content)
...
//        if ptModel.platId == 6 {
//            if ptModel.card != "" && ptModel.cash != "" && ptModel.topUp != ""  {
//                let to = (Double(ptModel.cash) ?? 0) + (Double(ptModel.card) ?? 0) + (Double(ptModel.topUp) ?? 0)
