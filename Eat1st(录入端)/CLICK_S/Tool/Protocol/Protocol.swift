...
//
    ...
    ///0为图片  1为视频  2为视频和图片
    func showPhotoLibrary(_ selCount: Int = 1, _ pickerType: Int = 0) {
        let currentVC = PJCUtil.currentVC()
        
        var config = PHPickerConfiguration()
        config.selectionLimit = selCount
        switch pickerType {
        case 0:
        ...
        picker.delegate = self as? any PHPickerViewControllerDelegate
...
//        }
//
//        let nav = HXCustomNavigationController(manager: phManager, delegate: self as? HXCustomNavigationControllerDelegate)
//        
